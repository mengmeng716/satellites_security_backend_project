...
def update_pipeline_stage(database_writer: Any | None, stage: str) -> None:
    if database_writer is not None and hasattr(database_writer, "update_stage"):
        database_writer.update_stage(stage)


def touch_pipeline_heartbeat(database_writer: Any | None) -> None:
    if database_writer is not None and hasattr(database_writer, "heartbeat"):
        database_writer.heartbeat()
...
def main(
    ...
    if not args.skip_simulation:
        update_pipeline_stage(database_writer, "Simulation")
        run_command(
            "SIMULATION",
            simulation_cmd,
            cwd=PROJECT_ROOT,
    ...
    if args.dry_run:
        ...
        if not args.skip_part3:
            update_pipeline_stage(database_writer, "SelfHealing")
            run_command(
                "PART3_REBUILD",
                part3_cmd,
                cwd=PROJECT_ROOT,
